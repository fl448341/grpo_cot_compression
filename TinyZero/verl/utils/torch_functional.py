...
# to get custom eos tokens from qwen
def get_eos_mask(response_id: torch.Tensor, eos_token=2, dtype=torch.int64):
    if isinstance(eos_token, (list, tuple)):
        eos_hit = torch.zeros_like(response_id, dtype=torch.bool)
        for t in eos_token:
            eos_hit = eos_hit | response_id.eq(t)
        eos_mask = eos_hit.long()
    else:
