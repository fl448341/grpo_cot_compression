...
class ActorRolloutRefWorker(Worker):
    ...
    def generate_sequences(self, prompts: DataProto):
        ...
        # Qwen ends with <|endoftext|> and tokenizer.eos is <|im_end|> so we need to count the qwen eos
        _eos = self.tokenizer.eos_token_id
        _pad = self.tokenizer.pad_token_id
        _eos_list = list({_eos, _pad}) if _pad is not None else _eos
        meta_info = {'eos_token_id': _eos_list, 'pad_token_id': _pad}
